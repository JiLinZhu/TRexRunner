...
Shape* dinoShapeCreate( ){
  Shape *s = (Shape*)malloc( sizeof(Shape) ); assert(s);
  s->numPoints = 4;
  s->points = (Vector*)malloc( sizeof(Vector) * 4 ); assert(s->points);
  s->normals = (Vector*)malloc( sizeof(Vector) * 4 ); assert(s->normals);

  s->points[0] = vectorCreate( 0, 0, 20, dinoY );
  s->points[1] = vectorCreate( 0, 0, 10+frontLeg_width, dinoY );
  s->points[2] = vectorCreate( 0, 0, 20, dinoY+frontLeg_height );
  s->points[3] = vectorCreate( 0, 0, 15, dinoY+frontLeg_height );

  for( int i = 0; i < 3; i++ )
    s->normals[i] = normVector( vectorCreate( s->points[i].x, s->points[i].y, s->points[i+1].x, s->points[i+1].y ) );
  s->normals[3] = normVector( vectorCreate( s->points[3].x, s->points[3].y, s->points[0].x, s->points[0].y ) );
  return s;
}

Shape* cactiShapeCreate( ){
  Shape *s = (Shape*)malloc( sizeof(Shape) ); assert(s);
  s->numPoints = 4;
  s->points = (Vector*)malloc( sizeof(Vector) * 4 ); assert(s->points);
  s->normals = (Vector*)malloc( sizeof(Vector) * 4 ); assert(s->normals);

  s->points[0] = vectorCreate( 0, 0, cactiX1, 47 );
  s->points[1] = vectorCreate( 0, 0, cactiX1 + smallcactus_width, 47 );
  s->points[2] = vectorCreate( 0, 0, cactiX1 + smallcactus_width, 47 + smallcactus_height );
  s->points[3] = vectorCreate( 0, 0, cactiX1, 47 +smallcactus_height );
...
void collisionDetectedSAT( Shape *x, Shape *y ){
  ...
  for( int i = 0; i < x->numPoints; i++ ) {
    ...
    if ( minProjX > maxProjY || minProjY > maxProjX ) {
      shapeDelete(x);
      shapeDelete(y);
      return;
    }
...
void loop(void){
  ...
  draw();
  collisionDetectedSAT( dinoShapeCreate(), cactiShapeCreate() );
}
